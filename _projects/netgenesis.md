---
layout: page
title: NET-GENESIS
description: European Commission, Marie Curie Felloship [completed]
img: assets/img/netgenesis_logo.png
importance: 2
category: PI
giscus_comments: false
---


### Aim
The **NET-GENESIS** (Network Micro-Dynamics in Emerging Technologies) project examines how networks form, evolve, and shape the development of emerging technologies. These networks involve interconnected actors—including individuals, firms, and institutions—operating across domains with different incentives and power structures, such as open science and market-based systems. The project explores how network configurations influence technological trajectories and the emergence of socially desirable innovations. Networks are particularly important in facilitating access to financial resources, supporting knowledge exchange within open innovation systems, and connecting scientific and technological domains to stimulate discovery and application development. Despite extensive research on the effects of networks on actor performance, limited attention has been paid to the origins and evolution of networks surrounding emerging technologies. NET-GENESIS addresses this gap through a comparative study of six case studies across the pharmaceutical, biotechnology, and nanotechnology industries, using a mixed qualitative-quantitative and multi-level research approach.


### Details
<table class="table table-borderless mt-4" style="max-width: 700px;">
  <tbody>
    <tr>
      <td style="width:20%; color:gray; font-size:0.85rem; text-transform:uppercase; letter-spacing:0.05em; vertical-align:top;">Funder</td>
      <td>European Commission<br>(Marie Cuire Fellowship, PIOF-GA-2012-331107)</td>
    </tr>
    <tr>
      <td style="color:gray; font-size:0.85rem; text-transform:uppercase; letter-spacing:0.05em; vertical-align:top;">Role</td>
      <td>Principal Investigator</td>
    </tr>
    <tr>
      <td style="color:gray; font-size:0.85rem; text-transform:uppercase; letter-spacing:0.05em; vertical-align:top;">Budget<br>(Period)</td>
      <td>£ 129,422<br>(Mar. 2014 – Mar. 2016)</td>
    </tr>
    <tr>
      <td style="color:gray; font-size:0.85rem; text-transform:uppercase; letter-spacing:0.05em; vertical-align:top;">Partners</td>
      <td>
        <strong>Ben Martin</strong> (SPRU, University of Sussex)<br>
        <strong>Diana Hicks</strong> (School of Public Policy, Georgia Institute of Technology
)
      </td>
    </tr>
  </tbody>
</table>

### Related publications
- {% reference Rotolo2015 %}<br>
<a href="https://doi.org/10.1016/j.respol.2015.06.006" target="_blank"><i class="fas fa-file-pdf"></i> Publisher</a> · <a href="https://dx.doi.org/10.2139/ssrn.2564094" target="_blank"><i class="fas fa-file-pdf"></i> Pre-print</a>
- {% reference Rotolo2015a %}<br>
<a href="https://doi.org/10.1002/asi.23385" target="_blank"><i class="fas fa-file-pdf"></i> Publisher</a> · <a href="https://ssrn.com/abstract=2742165" target="_blank"><i class="fas fa-file-pdf"></i> Pre-print</a>
- {% reference Leydesdorff2016a %}<br>
<a href="https://doi.org/10.1002/asi.23551" target="_blank"><i class="fas fa-file-pdf"></i> Publisher</a> · <a href="https://doi.org/10.48550/arXiv.1502.05676" target="_blank"><i class="fas fa-file-pdf"></i> Pre-print</a>
- {% reference Petersen2016 %}<br>
<a href="https://doi.org/10.1016/j.respol.2015.12.004" target="_blank"><i class="fas fa-file-pdf"></i> Publisher</a> · <a href="https://dx.doi.org/10.2139/ssrn.2604702" target="_blank"><i class="fas fa-file-pdf"></i> Pre-print</a>
- {% reference Grassano2017 %}<br>
<a href="https://doi.org/10.1002/asi.23737" target="_blank"><i class="fas fa-file-pdf"></i> Publisher</a> · <a href="https://doi.org/10.48550/arXiv.1604.04896" target="_blank"><i class="fas fa-file-pdf"></i> Pre-print</a>